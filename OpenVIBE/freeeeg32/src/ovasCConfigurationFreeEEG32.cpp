...
// #define TERM_SPEED 921600 // FreeEEG32 is a bit faster than others
 #define TERM_SPEED 1500000 // FreeEEG32 is a bit faster than others
...
#define MAXIMUM_SERIAL_USB_TTY (128-MAXIMUM_SERIAL_TTY)
#define MAXIMUM_SERIAL_ACM_TTY (256-MAXIMUM_SERIAL_USB_TTY)

uint32 OpenViBEAcquisitionServer::CConfigurationFreeEEG32::getMaximumTTYCount(void)
{
	return MAXIMUM_SERIAL_ACM_TTY + MAXIMUM_SERIAL_USB_TTY + MAXIMUM_SERIAL_TTY;
}

CString OpenViBEAcquisitionServer::CConfigurationFreeEEG32::getTTYFileName(const uint32 ui32TTYNumber)
{
	char l_sBuffer[1024];
#if defined TARGET_OS_Windows
	...
	else if(ui32TTYNumber<MAXIMUM_SERIAL_ACM_TTY)
	{
		::sprintf(l_sBuffer, "/dev/ttyACM%u", ui32TTYNumber-MAXIMUM_SERIAL_USB_TTY);
	}
	else
	{
		::sprintf(l_sBuffer, "/dev/ttyS%u", ui32TTYNumber-MAXIMUM_SERIAL_ACM_TTY);
	}
#else
	::sprintf(l_sBuffer, "");
#endif
...
bool OpenViBEAcquisitionServer::CConfigurationFreeEEG32::isTTYFile(const CString& sFileName)
{
#if defined TARGET_OS_Windows
...
static void checkbutton_daisy_module_cb(::GtkToggleButton* pButton, CConfigurationFreeEEG32* pConfigurationFreeEEG32)
{
	pConfigurationFreeEEG32->checkbuttonDaisyModuleCB(gtk_toggle_button_get_active(pButton)?CConfigurationFreeEEG32::DaisyStatus_Active:CConfigurationFreeEEG32::DaisyStatus_Inactive);
}

CConfigurationFreeEEG32::CConfigurationFreeEEG32(const char* sGtkBuilderFileName, OpenViBE::uint32& rUSBIndex)
...
CConfigurationFreeEEG32::~CConfigurationFreeEEG32(void)
{
	g_object_unref(m_pListStore);
}

boolean CConfigurationFreeEEG32::preConfigure(void)
{
	...
	for(uint32 i=0, j=1; i<CConfigurationFreeEEG32::getMaximumTTYCount(); i++)
	{
		CString l_sFileName = CConfigurationFreeEEG32::getTTYFileName(i);
		if(CConfigurationFreeEEG32::isTTYFile(l_sFileName))
		{
...
boolean CConfigurationFreeEEG32::postConfigure(void)
{
	::GtkComboBox* l_pComboBox=GTK_COMBO_BOX(gtk_builder_get_object(m_pBuilderConfigureInterface, "combobox_device"));
...
boolean CConfigurationFreeEEG32::setAdditionalCommands(const CString& sAdditionalCommands)
{
        m_sAdditionalCommands=sAdditionalCommands;
        return true;
}

CString CConfigurationFreeEEG32::getAdditionalCommands(void) const
{
        return m_sAdditionalCommands;
}

boolean CConfigurationFreeEEG32::setReadBoardReplyTimeout(uint32 iReadBoardReplyTimeout)
{
        m_iReadBoardReplyTimeout=iReadBoardReplyTimeout;
        return true;
}

uint32 CConfigurationFreeEEG32::getReadBoardReplyTimeout(void) const
{
        return m_iReadBoardReplyTimeout;
}

boolean CConfigurationFreeEEG32::setFlushBoardReplyTimeout(uint32 iFlushBoardReplyTimeout)
{
        m_iFlushBoardReplyTimeout=iFlushBoardReplyTimeout;
        return true;
}

uint32 CConfigurationFreeEEG32::getFlushBoardReplyTimeout(void) const
{
        return m_iFlushBoardReplyTimeout;
}

bool CConfigurationFreeEEG32::setDaisyModule(bool bDaisyModule)
{
        m_bDaisyModule=bDaisyModule;
        return true;
}

bool CConfigurationFreeEEG32::getDaisyModule(void) const
{
        return m_bDaisyModule;
}

void CConfigurationFreeEEG32::checkbuttonDaisyModuleCB(EDaisyStatus eStatus)
{
...
CConfigurationFreeEEG32::SDaisyInformation CConfigurationFreeEEG32::getDaisyInformation(EDaisyStatus eStatus)
{
